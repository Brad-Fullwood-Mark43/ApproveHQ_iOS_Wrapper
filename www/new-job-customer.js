...
(() => {
    ...
    <div id="inlineCustomerPanel" class="hidden" style="margin-top:14px;padding-top:14px;border-top:1px solid rgba(128,128,128,.25)">
      ...
      <div id="inlineCustomerFields">
        ...
        <div class="action-grid">
          <button id="createInlineCustomer" class="action-btn" type="button">Add & select customer</button>
  ...
  function clearFields() {
    ['inlineCustomerName', 'inlineCustomerPhone', 'inlineCustomerCompany', 'inlineCustomerEmail']
      .forEach((id) => {
  ...
  function setOpen(open) {
    ...
    if (jobSubmit) jobSubmit.disabled = open;
  ...
  button.addEventListener('click', createInlineCustomer);

  panel.addEventListener('keydown', (event) => {
    if (event.key !== 'Enter' || event.shiftKey) return;
    event.preventDefault();
    event.stopPropagation();
    if (!button.disabled) createInlineCustomer();
  });

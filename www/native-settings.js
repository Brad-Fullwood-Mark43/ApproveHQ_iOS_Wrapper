...
(function(){
  ...
  async function registerPush(){const p=await plugin(),s=$('pushTestStatus');if(!p){s.textContent='Native bridge unavailable.';return;}s.textContent='Requesting permission and registering this iPhone…';try{const permission=await p.requestPushPermission({});if(!permission?.granted){s.textContent='Notifications are not allowed. You can enable them in iPhone Settings.';await refresh();return}let token;for(let i=0;i<16;i++){token=await p.getPushToken({});if(token?.deviceToken)break;await sleep(250)}if(!token?.deviceToken)throw new Error('Apple has not returned a device token yet. Wait a moment and tap again.');const x=await post('/api/mobile/push/register',{deviceToken:token.deviceToken,environment:token.environment||'production'});if(!x.response.ok)throw new Error(x.data?.error||'Server registration failed.');s.textContent='Push notifications registered ✓';await refresh();}catch(e){s.textContent=e?.message||'Could not register notifications.';}}
